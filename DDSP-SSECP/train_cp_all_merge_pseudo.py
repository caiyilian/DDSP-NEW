...
class DDSPSeg(object):
    ...
    def compute_fea_loss(self):
        ...
        # ========================== PACCA & Structural Loss ==========================
        if self.proto_bank.initialized.sum() > 0:
            # 1. Compute Centroids for Target Domain (Original)
            # pseudo_label: (1, C, D, H, W), tar_mask: (D, H, W) -> unsqueeze to (1, D, H, W)
            tar_centroids = compute_centroids(self.latent_b, self.pseudo_label, self.tar_mask.unsqueeze(0))
            
            # 2. Compute Centroids for Target Domain (CP Augmented)
            # pseudo_label_tar_cp: (1, C, D, H, W), tar_mask_cp: (D, H, W)
            tar_cp_centroids = compute_centroids(self.latent_b_cp, self.pseudo_label_tar_cp, self.tar_mask_cp.unsqueeze(0))
            
            # 3. Calculate Contrastive Loss (PACCA)
            pacca_loss_b = prototype_contrastive_loss(tar_centroids, self.proto_bank.prototypes)
            pacca_loss_b_cp = prototype_contrastive_loss(tar_cp_centroids, self.proto_bank.prototypes)
            
            # 4. Calculate Structural Consistency Loss
            struct_loss_b = structural_consistency_loss(tar_centroids, self.proto_bank.prototypes)
            struct_loss_b_cp = structural_consistency_loss(tar_cp_centroids, self.proto_bank.prototypes)
            
            # Add to total feature loss
            # Use consistency weight to ramp up the influence
            pacca_weight = self.csco * self.consist_weight
            self.fe_loss += pacca_weight * (pacca_loss_b + pacca_loss_b_cp)
            self.fe_loss += pacca_weight * (struct_loss_b + struct_loss_b_cp)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoint_root', default="./checkpoint/all_merge_pseudo_pro12")
