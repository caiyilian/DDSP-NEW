...
class DDSPSeg(object):
    def __init__(self, args=None):
        ...
        # initialize model
        self.enc = UNet_base(input_channels=1).to(self.device)
        self.seg = Seg(out=self.n_classes).to(self.device)
    ...
    def forward_seg(self):
        ...
        pred_mask_a_cp = self.seg(self.latent_a_cp)
        pred_mask_b_cp = self.seg(self.latent_b_cp)
        pred_mask_a_r_cp = self.seg(self.latent_a_r_cp)
        pred_mask_b_r_cp = self.seg(self.latent_b_r_cp)

        self.pred_mask_a_cp_back = pred_mask_a_cp * (1 - self.img_boxes) + self.img_boxes * pred_mask_b_cp
        self.pred_mask_b_cp_back = pred_mask_b_cp * (1 - self.img_boxes) + self.img_boxes * pred_mask_a_cp
        self.pred_mask_a_r_cp_back = pred_mask_a_r_cp * (1 - self.img_boxes) + self.img_boxes * pred_mask_b_r_cp
        self.pred_mask_b_r_cp_back = pred_mask_b_r_cp * (1 - self.img_boxes) + self.img_boxes * pred_mask_a_r_cp
    ...
    def compute_seg_loss(self):
        ...
        self.seg_loss = self.L_seg(self.pred_mask_real_a, self.srs_label) + self.L_seg(self.pred_mask_real_a_r, self.srs_label) + \
                        0.1 * (self.L_seg(self.pred_mask_a_cp_back, self.srs_label) + self.L_seg(self.pred_mask_a_r_cp_back, self.srs_label))

        self.target_consist = 0.1 * (self.L_seg(self.pred_mask_b, self.pred_mask_b_r) +
                        0.5 * (self.L_seg(self.pred_mask_b, self.pred_mask_b_cp_back) + self.L_seg(self.pred_mask_b, self.pred_mask_b_r_cp_back)))
    ...
    def train_epoch(self, epoch):
        ...
        for i in range(self.iters):
            ...
            srsimg = srsimg.to(self.device)
            srsimg_r = srsimg_r.to(self.device)
            tarimg = tarimg.to(self.device)
            tarimg_r = tarimg_r.to(self.device)
            srslabel = srslabel.to(self.device)
            ...
            srslabel = torch.from_numpy(self.to_categorical(srslabel, num_classes=self.n_classes)[np.newaxis, :, :, :, :]).to(self.device)
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--A_root', default="./mmwhs/mmwhs/ct_processing")
    # parser.add_argument('--B_root', default="./mmwhs/mmwhs/mr_processing")
    ...
    parser.add_argument('--checkpoint_root', default="./checkpoint/ablation/module ablation/CPR")
