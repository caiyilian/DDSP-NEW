...
class DDSPSeg(object):
    def __init__(self, args=None):
        ...
        self.csco = args.csco
        self.threshold = 0.95
        self.loss_p = args.loss_proportion
        self.best_dice = 0
        ...
        if args.resume:
            self.load_model(args.load_path, args.start_epoch)
    ...
    def compute_fea_loss(self):
        ...
        self.fe_loss = self.csco * (src_cs_loss + self.loss_p * (
                    tar_cs_loss + self.loss_p * (src_cp_cs_loss + tar_cp_cs_loss)))
    ...
    def compute_seg_loss(self):  # 改完一半，下一步改这里，应当套入基于mask的损失计算
        ...
        # 利用伪标签的无监督分割损失
        self.unsup_loss_s_cp = self.L_seg(self.pred_mask_b, self.pseudo_label, self.tar_mask)
        ...
        self.seg_loss = (self.sup_seg_loss +
                         self.loss_p * (self.unsup_loss_src_cp + self.unsup_loss_tar_cp + self.unsup_loss_s_cp +
                         self.loss_p * ( self.unsup_loss_src_r_cp + self.unsup_loss_tar_r_cp)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--loss_proportion', type=float, default=0.5)
    ...
    parser.add_argument('--checkpoint_root', default="./checkpoint/DDSP_with_cp_aug_pro12")
    parser.add_argument('--device', default='cuda:1', help='device id (i.e. 0 or 0,1 or cpu)')

    parser.add_argument('--load_pretrain', default=False, help='load pretrain teacher or not')
    parser.add_argument('---pretrain_ckpt_path', default='./checkpoint/ablation/loss_ablation/loss_p_0.5', help='load path of pretrained ckpt')
    parser.add_argument('--resume', default=False, help='resume or not')
    parser.add_argument('--load', default=None, help='load path of ckpt for resume')
