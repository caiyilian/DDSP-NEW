...
class AffineTransformer(nn.Module):
    ...
    def forward(self, src, mat, mode='bilinear'):
        src = src.float()
        norm = torch.tensor([[1, 1, 1, src.shape[2]], [1, 1, 1, src.shape[3]], [1, 1, 1, src.shape[4]]], dtype=torch.float).to(self.device)
        # norm = torch.tensor([[1, 1, 1, src.shape[2]], [1, 1, 1, src.shape[3]], [1, 1, 1, src.shape[4]]], dtype=torch.float)
...
class SpatialTransform(object):
    def __init__(self, device, do_rotation=True, angle_x=(-np.pi / 12, np.pi / 12), angle_y=(-np.pi / 12, np.pi / 12),
        ...
        self.atn = AffineTransformer(device)
    ...
    def rand_coords(self, patch_size):
        coords = torch.from_numpy(self.create_zero_centered_coordinate_mesh(patch_size)).to(self.device)
        mat = torch.from_numpy(np.identity(3, dtype=np.float32)).to(self.device)
        # coords = torch.from_numpy(self.create_zero_centered_coordinate_mesh(patch_size))
        ...
        ctr = torch.FloatTensor([patch_size[0] // 2, patch_size[1] // 2, patch_size[2] // 2]).to(self.device)
        # ctr = torch.FloatTensor([patch_size[0] // 2, patch_size[1] // 2, patch_size[2] // 2])
    ...
    def rotate_mat(self, mat, angle_x, angle_y, angle_z):
        rot_mat_x = torch.FloatTensor([[1, 0, 0], [0, np.cos(angle_x), -np.sin(angle_x)], [0, np.sin(angle_x), np.cos(angle_x)]]).to(self.device)
        rot_mat_y = torch.FloatTensor([[np.cos(angle_y), 0, np.sin(angle_y)], [0, 1, 0], [-np.sin(angle_y), 0, np.cos(angle_y)]]).to(self.device)
        rot_mat_z = torch.FloatTensor([[np.cos(angle_z), -np.sin(angle_z), 0], [np.sin(angle_z), np.cos(angle_z), 0], [0, 0, 1]]).to(self.device)
        # rot_mat_x = torch.FloatTensor([[1, 0, 0], [0, np.cos(angle_x), -np.sin(angle_x)], [0, np.sin(angle_x), np.cos(angle_x)]])
        # rot_mat_y = torch.FloatTensor([[np.cos(angle_y), 0, np.sin(angle_y)], [0, 1, 0], [-np.sin(angle_y), 0, np.cos(angle_y)]])
        # rot_mat_z = torch.FloatTensor([[np.cos(angle_z), -np.sin(angle_z), 0], [np.sin(angle_z), np.cos(angle_z), 0], [0, 0, 1]])

        mat = torch.matmul(rot_mat_z, torch.matmul(rot_mat_y, torch.matmul(rot_mat_x, mat)))
        return mat

    def deform_coords(self, coords, alpha, sigma):
        offsets = torch.rand(coords.shape).to(self.device)*2 -1
        # offsets = torch.rand(coords.shape) * 2 - 1
        ker1d = self._gaussian_kernel1d(sigma).astype(np.float32)[np.newaxis, np.newaxis, :]
        ker1d = torch.from_numpy(ker1d).to(self.device)
        # ker1d = torch.from_numpy(ker1d)
    ...
    def scale_mat(self, mat, scale_x, scale_y, scale_z):
        scale_mat = torch.FloatTensor([[scale_x, 0, 0], [0, scale_y, 0], [0, 0, scale_z]]).to(self.device)
        # scale_mat = torch.FloatTensor([[scale_x, 0, 0], [0, scale_y, 0], [0, 0, scale_z]])
    ...
    def shear_mat(self, mat, shear_xy, shear_xz, shear_yx, shear_yz, shear_zx, shear_zy):
        shear_mat = torch.FloatTensor([[1, np.tan(shear_xy), np.tan(shear_xz)], [np.tan(shear_yx), 1, np.tan(shear_yz)],
                     [np.tan(shear_zx), np.tan(shear_zy), 1]]).to(self.device)
        # shear_mat = torch.FloatTensor([[1, np.tan(shear_xy), np.tan(shear_xz)], [np.tan(shear_yx), 1, np.tan(shear_yz)],
    ...
    def translate_mat(self, mat, trans_x, trans_y, trans_z):
        trans = torch.FloatTensor([trans_x, trans_y, trans_z]).to(self.device)
        # trans = torch.FloatTensor([trans_x, trans_y, trans_z])
