...
class Dataset3D_remap(data.Dataset):
    def __init__(self, dir_, rmmax=50, trans_type="Remap", aug_times=2):
    ...
    def __getitem__(self, index):
        ...
        #img = pre(img, [-100, 800])  ##CT   怎么这样写代码啊......
        img = pre(img)
        assert np.min(img) >= -1
        assert np.max(img) <= 1
        ...
        img_r_list = []
        for i in range(self.aug_t):
            img_r = torch.from_numpy(img)
            img_r = self.func(img_r)    # 每次getitem调用，得到的img_r都不一样
            img_r_list.append(img_r)

        # aug_img_iter = Aug_Img_Iter(img, self.func, self.aug_t)

        return img, img_r_list, label

    def __len__(self):
        return len(self.filenames)


class Dataset3D_remap_src(data.Dataset):
    def __init__(self, dir_, rmmax=50, trans_type="Remap"):
        super(Dataset3D_remap_src, self).__init__()
        self.filenames = dir_
        if trans_type == "Remap":
            self.func = Remap(isremap=True, p=0, rmmax=rmmax)
        else:
